...
cv::Mat frame_split_stitch(cv::Mat& frame)
{
    ...
    cv::Rect quad2(0, line1, cols, line1); // strip2
    cv::Rect quad3(0, line2, cols, line1); // strip3
    cv::Rect quad4(0, line3, cols, line1); // strip4
...
{	
    ...
    for (i = 0; i < imageHeight; i++)
    {
        for (j = 0; j < imageWidth; j+=8)
        {
            ...
            {
				uint8_t *pixel = rgbImage->ptr<uint8_t>(i, j); // Vec<uchar, 3>
